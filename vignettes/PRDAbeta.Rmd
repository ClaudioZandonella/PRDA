...
{PRDAbeta} package can be used for Pearson's correlation between two variables or mean comparisons (i.e., one-sample, paired, two-sample, and Welch's t-test) considering an hypothetical value of $\rho$ or Cohen's *d* respectively. See [`vignette("retrospective")`](retrospective.html) and [`vignette("prospective")`](prospective.html) to know how to set function arguments for the different effect types. 
