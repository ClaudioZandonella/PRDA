...
Arguments complete description is provided in the function help page `?retrospective`. In the following sections, instead, different examples are presented. For further details about design analysis see @altoeEnhancingStatisticalInference2020 and @bertoldoDesigningStudiesEvaluating2020.
...
The function `retrospective()` returns an object of class `"design_analysis"` that contains:
...
- `retrospective_res` - a data frame with the resulting inferential errors.
...
Note that the inferential risks associated to the *i* value of the `effect_samples` vector are reported at the *i* row of `retrospective_res` dataframe. Thus, we can simply add `effect_samples` as a new column of the dataframe.
...
Plotting the distribution of sampled effects, we can evaluate whether their accurately represent the intended distribution. If not we could  increase the number of sampled effects.
